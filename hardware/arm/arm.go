...
import (
	...
	"github.com/jetsetilly/test7800/hardware/arm/rng"
	"github.com/jetsetilly/test7800/hardware/arm/timer"
...
func NewARM(mmap architecture.Map, mem SharedMemory, hook CartridgeHook) *ARM {
	arm := &ARM{
		mmap:           mmap,
		mem:            mem,
		hook:           hook,
	...
	arm.state.mam = newMam(arm.mmap)
	arm.state.rng = rng.NewRNG(arm.mmap)
	if arm.mmap.HasT1 {
...
func (arm *ARM) updatePrefs() {
	// THIS FUNCTION SETS CONSTANT VALUES FOR WHAT SHOULD BE PREFERENCE VALUES.
	// IN THE ORIGINAL GOPHER2600 VERSION THIS WAS CONNECTED TO A PROPER
	// PREFERENCE SYSTEM. THAT'S MISSING FOR NOW IN TEST7800
...
func (arm *ARM) logYield() {
	...
	if arm.state.yield.Error != nil {
		logger.Logf(logger.Allow, "ARM7", "%s: %v", arm.state.yield.Type, arm.state.yield.Error)
	} else {
		logger.Logf(logger.Allow, "ARM7", "%s: no specific error", arm.state.yield.Type)
	}

	// extended memory logging
	const extendedMemoryLogging = false
	if !extendedMemoryLogging {
		return
	}
	...
	if entry != nil {
		logger.Log(logger.Allow, "ARM7", entry)
		logger.Log(logger.Allow, "ARM7", arm.disasmVerbose(*entry))
	}
}
