...
func newElfMemory(ctx Context) *elfMemory {
	mem := &elfMemory{
		ctx:            ctx,
		gpio:           newGPIO(),
	...
	for _, rel := range ef.Sections {
		...
		for i := 0; i < len(relData); i += 8 {
			...
			case elf.R_ARM_ABS32:
				...
				logger.Logf(logger.Allow, "ELF", "%s %s (%08x) => %08x", typ, name, secBeingRelocated.origin+offset, tgt)
			...
			case elf.R_ARM_REL32:
				if sym.Section == elf.SHN_UNDEF {
					logger.Logf(logger.Allow, "ELF", "REL32 section is undefined")
					continue
				}
				...
				logger.Logf(logger.Allow, "ELF", "REL32 %s (%08x) => %08x", sym.Name, secBeingRelocated.origin+offset, tgt)

			case elf.R_ARM_PREL31:
				if sym.Section&0xff00 == elf.SHN_UNDEF {
					logger.Logf(logger.Allow, "ELF", "PREL31 section is undefined")
					continue
				}
				return fmt.Errorf("ELF: PREL31 not fully supported")

			case elf.R_ARM_THM_JUMP24:
				// this relocation type is very similar to R_ARM_THM_PC22. the
				// difference is the base address for the upper half of the
				// opcode. in the case of THM_JUMP24 it is 0xf800 (instead of 0xd000)
				...
				lo := uint16(0xf000 | (s << 10) | imm10)
				hi := uint16(0xf800 | (j1 << 13) | (j2 << 11) | imm11)
				opcode := uint32(lo) | (uint32(hi) << 16)
				...
				logger.Logf(logger.Allow, "ELF", "THM_JUMP24 %s (%08x) => opcode %08x", name, secBeingRelocated.origin+offset, opcode)
...
func (mem *elfMemory) mapAddress(addr uint32, write bool) (*[]byte, uint32) {
	...
	if addr >= mem.gpio.dataOrigin && addr <= mem.gpio.dataMemtop {
		if mem.stream.active {
			logger.Logf(logger.Allow, "ELF", "disabling byte streaming: %d bytes in stream", mem.stream.ptr)
