...
func vcsInitNvStore(mem *elfMemory) {
	...
	p, err := resources.JoinPath(elf_nvram, mem.md5sum)
	if err != nil {
		logger.Log(logger.Allow, "ELF", err)
		return
	}
	...
	if st.Size() != sz {
		logger.Logf(logger.Allow, "ELF", "%s is not %d bytes in size", p, sz)
		return
	}
	...
	for _, sec := range mem.nv.sections {
		...
		if data == nil {
			logger.Logf(logger.Allow, "ELF", "cannot find memory for address %#08x", sec.addr)
			return
		}
...
func vcsWriteNvChunk(mem *elfMemory) {
	if !mem.nv.initialised {
		logger.Log(logger.Allow, "ELF", "NV memory is not initialised")
		return
	}
	...
	for _, sec := range mem.nv.sections {
		...
		if n != int(sec.size*4) {
			logger.Log(logger.Allow, "ELF", "incorrect number of bytes written to NV file")
			return
		}
	}
}
