...
func (ee *EEPROM) Read() {
	...
	if err != nil {
		logger.Logf(ee.ctx, "savekey", "could not load eeprom file: %v", err)
		return
	}
	...
	if fs.Size() != int64(len(ee.Data)) {
		logger.Logf(ee.ctx, "savekey", "eeprom file is of incorrect length. %d should be 65536 ", fs.Size())
	}
	...
	logger.Logf(ee.ctx, "savekey", "eeprom file loaded from %s", fn)
}

func (ee *EEPROM) Write() {
	fn, err := resources.JoinPath(saveKeyPath)
	if err != nil {
		logger.Logf(ee.ctx, "savekey", "could not write eeprom file: %v", err)
		return
	}
	...
	defer func() {
		err := f.Close()
		if err != nil {
			logger.Logf(ee.ctx, "savekey", "could not close eeprom file: %v", err)
		}
	}()
	...
	if n != len(ee.Data) {
		logger.Logf(ee.ctx, "savekey", "eeprom file has not been truncated during write. %d should be 65536", n)
		return
	}

	logger.Logf(ee.ctx, "savekey", "eeprom file saved to %s", fn)
